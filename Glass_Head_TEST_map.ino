...
// process the color mode select switch - Touch center
  ...
  switch (switchesNow.kniferight) {
    case 0:
     ...
     for (int i = 71; i > -1; i--){
       strip.setPixelColor(Right2Left[i] - 1, 0, 0, 0);
       strip.show();   //show the new strip colors - FYI - takes rough 4.66 milliseconds on the Uno
       delay(10);
     }  
     break;
    ...
    case 1:
     for (int i = 0; i < 72; i++){
       strip.setPixelColor(Bottom2Top[i] - 1, colorR[colorMode], colorG[colorMode], colorB[colorMode]);
       strip.show();   //show the new strip colors - FYI - takes rough 4.66 milliseconds on the Uno
       delay(10);
     }
     for (int i = 71; i > -1; i--){
       strip.setPixelColor(Bottom2Top[i] - 1, 0, 0, 0);
       strip.show();   //show the new strip colors - FYI - takes rough 4.66 milliseconds on the Uno
       delay(10);
     }  
     break;
